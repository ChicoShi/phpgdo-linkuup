...
allbars=[e for e in raw['elements'] if e.get('tags',{}).get('amenity') in ('bar','pub')]
...
for e in allbars:
 ...
 if not name or any(k in t for k in ('disused','abandoned','demolished','end_date')):reject('inactive_or_unnamed');continue
 if e['type']=='way' and ring_for(e):b=e;ring=ring_for(e);original=None
 elif e['type']=='node':
  original=[e['lon'],e['lat']]
  matches=[(b,r) for b,r in outlines if contains(original,r)]
  if len(matches)!=1:reject('no_unique_containing_building');continue
  b,ring=matches[0]
 else:reject('unsupported_geometry');continue
 if any((x['type'],x['id'])!=(e['type'],e['id']) and x['type']=='node' and contains([x['lon'],x['lat']],ring) for x in allbars):reject('multiple_bars_in_building');continue
...
out=root/'data/bar-expansion';out.mkdir(exist_ok=True)
